		...
		for name, v := range volumes {
			...
			if err != nil {
				...
				if !volume.IsNotSupported(err) {
					glog.V(4).Infof("Failed to calculate volume metrics for pod %s volume %s: %+v", format.Pod(pod), name, err)
				}
				continue
			}
			...
			if !ok {
				glog.Errorf("Volume %s is not the volume of Pod %s", name, pod.UID)
				continue
			} else {
				if volSource.EmptyDir != nil && volSource.EmptyDir.Medium == v1.StorageMediumDefault {
					...
					glog.V(6).Infof("volume %s is not emptydir or medium is not default", name)
				}
			}
